...
3. **Faculty Approval**: The request enters `PENDING_APPROVAL`. The Faculty Mentor assigned to the club must approve the transfer. If no Faculty Mentor is currently assigned to the club, any `Faculty Admin` may approve the transfer as a fallback.
...
## Faculty Admin Fallback
If a club temporarily has no assigned Faculty Mentor (e.g., the professor leaves the university), the `approve_leadership_transfer` RPC accepts approval from any user with the `FACULTY_ADMIN` global role. This prevents handover requests from being stuck in `PENDING_APPROVAL` indefinitely when the current student president graduates.

## Platform Override
If the Club Admin unexpectedly leaves, the `Platform Admin` can forcefully execute a `FORCE_TRANSFER_LEADERSHIP` RPC.

## Failure Scenarios
If the Faculty Mentor (or Faculty Admin fallback) rejects the transfer, the initiator remains Club Admin.
